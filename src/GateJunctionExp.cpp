...
void GateJunctionExp::process(const ProcessArgs &args)
{
    ...
    for(int i = 0; i < 8; i++)
    {
        ...
        if (muteTrigger[i].process(muteGate ? 5.0f : 0.0f)) muteState[i] ^= true;
        if (invTrigger[i].process(invertGate ? 5.0f : 0.0f)) invState[i] ^= true;

        if(muteState[i]) ins[i] = 0.0f;
        else if(invState[i]) ins[i] = 5.0f - ins[i];
